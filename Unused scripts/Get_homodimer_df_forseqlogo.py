...
homodimers = pd.read_csv("e:/SAMcc/output/parCC_homodimers_dictR .csv", header=None)
# print(homodimers)
print(pdb_scan)

# make dataframe by integrating all information of homodimers

homodimerdf = pd.DataFrame(columns=["pdbID", "bundleID", "Molecule", "gene", "organism", "CC_length", "from_p", "to_p", "chain1", "chain2", "poschain1", "poschain2", "pub_Title"])
...
for bundle in homodimers[0]:
    ...
    pos_chain1 = homodimers[3][homodimers[0] == bundle]
    pos_chain2 = homodimers[4][homodimers[0] == bundle]
    ...
    homodimerlist = [pdb, bundleID, molecule, gene, organism, length.values[0], from_p.values[0], to_p.values[0],
                     chain1.values[0], chain2.values[0], pos_chain1.values[0], pos_chain2.values[0], pub_title]
    print(homodimerlist)
    df_length = len(homodimerdf)
    homodimerdf.loc[df_length] = homodimerlist

print(homodimerdf)
homodimerdf.to_csv("e:/SAMcc/output/parCC_homodimers_dfR .csv", index=False)

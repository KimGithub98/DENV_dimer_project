# import files
pdb_scan = open("e:/SAMcc/pdb_scan_topology_segments_20200717.csv", "r") ##pdb_scan_nr_topology_segments_20200717.csv

#get CCs
all_antiCC = []
for bundle in pdb_scan:
    bundle_split = bundle.split(',')
    top = bundle_split[6] #number of chains and bundle type 2_0 is antiparallel CC dimer, 2_1 parallel
...
for bundles in all_antiCC:
    ...
    try:
        with open(os.path.join("e:/SAMcc/pdb_scan_csv_redundant", folder, filename)) as f:
...
(pd.DataFrame.from_dict(data=homodimers, orient='index')
   .to_csv("e:/SAMcc/output/parCC_homodimers_dictR .csv", header=False))

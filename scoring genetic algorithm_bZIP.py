...
#variables
run_name = "bZIP_Non_Relative_Scores"
...
def objective(aa_code, register):
    ...
    I5_PCC2_score = 0
    I6_PCC2_score = 0
    int_weight_APCC = {"I1": 1, "I2_P": 0.25, "I3_P": 0.25, "I2_A": 0.5, "I3_A": 0.5, "I4": 1, "I5": 1, "I6": 1}
    int_weight_PCC = {"I1": 1, "I2_P": 0.5, "I3_P": 0.5, "I2_A": 0.25, "I3_A": 0.25, "I4": 1, "I5": 1, "I6": 1}
    for orientation_in in ["APCC", "PCC"]: #projects sequence on APCC and PCC structure
        ...
        if orientation_in == "APCC":
            cols = ["AA", "CCplus_APCC_asAPCC", "CCplus_PCC_asAPCC"] #relative probabilities that are taken into account
            for int in int_list.keys(): #looks at relative probabilities of an interaction observed in true APCCs
                AA_scores = Int_NR_scoring_matrix[Int_NR_scoring_matrix["Int"] == int][cols]
                ...
                for AA_comb in int_list[int]:
                    ...
                    if int == "I1":
                        ...
                        I1_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I2_P":
                        I2_P_APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I2_P_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I3_P":
                        I3_P_APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I3_P_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I2_A":
                        I2_A_APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I2_A_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I3_A":
                        I3_A_APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I3_A_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I4":
                        I4_APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I4_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I5":
                        I5_APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I5_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I6":
                        I6_APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I6_PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    APCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])*int_weight
                    PCC1_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])*int_weight
        elif orientation_in == "PCC":
            cols = ["AA", "CCplus_APCC_asPCC", "CCplus_PCC_asPCC"]
            for int in int_list.keys():
                ...
                for AA_comb in int_list[int]:
                    num_int_PCC += 1
                    if int == "I1":
                        I1_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I1_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I2_P":
                        I2_P_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I2_P_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I3_P":
                        I3_P_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I3_P_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I2_A":
                        I2_A_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I2_A_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I3_A":
                        I3_A_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I3_A_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I4":
                        I4_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I4_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I5":
                        I5_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I5_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    elif int == "I6":
                        I6_APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])
                        I6_PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])
                    PCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[1:2]].values[0])*int_weight#/PCC_neg_sel_weight # a high score means often observed in PCC compared to APCC
                    APCC2_score += sum(AA_scores[AA_scores["AA"] == AA_comb][cols[2:3]].values[0])*int_weight#/PCC_neg_sel_weight
    AA_total_weight = 0
...
i_pop = 0
d = []
for filename in os.listdir('e:/SAMcc/bZIP affinity test/sequence_files_half'):
#for CC in pop:
    i_pop += 1
    seq_file = open("e:/SAMcc/bZIP affinity test/sequence_files_half/" + filename, "r")
    reg_file = open("e:/SAMcc/bZIP affinity test/register_files_half/" + filename, "r")
    #seq_file = open("e:/SAMcc/output/genetic_algorithm_output/Sequence_files_random/CC" + str(i_pop) + ".txt", "w")
    ...
    seq_file.close()
    reg_file.close()
    last_gen_score = objective(CC, reg, PCC_neg_sel_weight)
    d.append(
        {
            'pdb' : filename[:-4],
            'aa_seq': CC,
            ...
            'APCC1_score' : last_gen_score[5],
            'PCC1_score': last_gen_score[6],
            'APCC2_score': last_gen_score[7],
            'PCC2_score': last_gen_score[8],
...
df = pd.DataFrame(d)
print(d)
df.to_csv("e:/SAMcc/output/genetic_algorithm_output/" + run_name + "_RE_permutations.csv", index=False)
